...
class ImageBook {
  ...
  Future<void> uploadLocalImageToFirestore(List<String> imagePaths) async {
    try {
  ...
  Future<void> getImageUrlsAndUpload() async {
    try {
      // Firebase Storageのリファレンスを取得する
      firebase_storage.Reference storageRef =
          firebase_storage.FirebaseStorage.instance.ref();

      // 指定したフォルダ内のファイルをリストアップする
      String downloadUrl_Head =
          await storageRef.child('parts/language_0.png').getDownloadURL();

      String downloadUrl_body =
          await storageRef.child('parts/socialsciences_1.png').getDownloadURL();

      String downloadUrl_foot =
          await storageRef.child('parts/generalities_2.png').getDownloadURL();
      ...
      // 画像アップロード処理を実行
      await uploadLocalImageToFirestore(imageUrls);
    } catch (e) {
